...
- Personal preference: run `uv run pytest --lf` (last-failed) to re-run only the tests that failed in the previous run — saves time when iterating on a fix.
...
- Personal note: Avoid deeply nested conditionals — flatten with early returns to improve readability.
...
- **Performance**: Use list accumulation for